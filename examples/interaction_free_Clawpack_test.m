...
tic
data = pyrunfile("myelasticity.py", "output", problem=2);
toc
...
Nsteps = 850;
tf = 35;
x0 = linspace(xclaw(1),xclaw(end),120*Nsteps);
t0 = tclaw;
dt = t0(2)-t0(1);
...
%%
f = figure; f.Position=[680 458 1200 380];
tiledlayout(2,2)
nexttile([2,1])
plot(xnumeric(end,:),uxnumeric(I(k),:),color=coloru); hold on
plot(xclaw,uxclaw(end,:),color=coloralt); hold off
ylim tight; xlim(tsub(k)+[-2.5,3.2])
legend("Characteristic system", "Clawpack", Location="northwest")
title("$t="+t(end)+"$")
nexttile
plot(xnumeric(end,:),uxnumeric(I(k),:),color=coloru); hold on
plot(xclaw,uxclaw(end,:),color=coloralt); hold off
xlim(tsub(k)+[0.7,0.85])
